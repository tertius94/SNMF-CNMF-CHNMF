...
# testando com o data set ionosphere
# fetch dataset
...
basis = 3
for i in range (n):
  mdl = CNMF(X_array, num_bases=basis)
...
basis = 3
for i in range (n):
  mdl = CHNMF(X_array, num_bases=basis)
  mdl.factorize(niter=10)
  media_linha = np.mean(mdl.H, axis=1)
  for j in range (basis):
   mdl.H = np.where(mdl.H[j] < media_linha[j]*0.001, 0, mdl.H)
...
#%%
# testando com o data set de dados urbanos
...
print("método SNMF: Dados Urbanos")
print("sparsity: %s" % sparsity)
...
basis = 15
for i in range (n):
  dados_mdl = CNMF(dados_urbanos, num_bases=basis)
  dados_mdl.factorize(niter=10)
  media_linha = np.mean(dados_mdl.H, axis=1)
  for j in range (basis):
   dados_mdl.H = np.where(dados_mdl.H[j] < media_linha[j]*0.001, 0, dados_mdl.H)
...
print("método CNMF: Dados Urbanos")
print("sparsity: %s" % sparsity)
print("norma de frobenius: %s" % dados_mdl.ferr)
print("percentagem dos dados que é explicada pela fatoração: %s%% \n" % qual_fact)
...
basis = 15
for i in range (n):
  dados_mdl = CHNMF(dados_urbanos, num_bases=basis)
  dados_mdl.factorize(niter=10)
  media_linha = np.mean(dados_mdl.H, axis=1)
  for j in range (basis):
   dados_mdl.H = np.where(dados_mdl.H[j] < media_linha[j]*0.001, 0, dados_mdl.H)
...
print("método CHNMF: Dados Urbanos")
print("sparsity: %s" % sparsity)
print("norma de frobenius: %s" % dados_mdl.ferr)
print("percentagem dos dados que é explicada pela fatoração: %s%% \n" % qual_fact)
